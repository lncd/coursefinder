...
class Courses_model extends CI_Model
{
	...
	*/
	function get_course_overviews($id_array)
	{
		...
		{
			$results = json_decode(file_get_contents($_SERVER['CF_N2_ENDPOINT'] . 'programmes/course_code/' . $a_key . '?access_token=' . $_SERVER['N2_TOKEN']));
	...
	*/
	...
	{
		return array('data' => json_decode(file_get_contents($_SERVER['CF_N2_ENDPOINT'] . 'programmes/course_code/' . $course_id . '?access_token=' . $_SERVER['N2_TOKEN'])), 'recommended' => $this->check_recommended($course_id, $this->session->userdata('search_id')));
	}
	...
	*/
	function get_similar_courses($course_id, $relevance = 0.5)
	{
		...
		{
			$course_info = json_decode(file_get_contents($_SERVER['CF_N2_ENDPOINT'] . 'programmes/course_code/' . $course->stored->target_course_id . '?access_token=' . $_SERVER['N2_TOKEN']));
